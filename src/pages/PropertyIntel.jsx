...
export default function PropertyIntel() {
  ...
  const generate = () => { if (inputAddr) setShowCapture(true); };

  const handleIntelSubmit = async () => {
    if (!email || !firstName) { setError("Name and email required"); return; }
    setSubmitting(true); setError("");
    await submitToHubSpot({ firstName, email, phone, source: "Property Intel Report", address: inputAddr });
    setSubmitting(false); setShowCapture(false); setGenerated(true);
  };
  ...
  return (
    <div style={{maxWidth:1200,margin:"0 auto",padding:"32px 32px 60px"}}>
      ...
      ) : showCapture && !generated ? (
        <Card style={{padding:32,maxWidth:480}}>
          <Lbl style={{marginBottom:6}}>Almost There</Lbl>
          <div style={{fontSize:15,fontWeight:700,color:T.text,marginBottom:4}}>Get your free Intelligence Report for:</div>
          <div style={{fontSize:14,color:T.brand,fontWeight:800,marginBottom:16}}>{inputAddr} · {hood.name}</div>
          <input value={firstName} onChange={e=>setFirstName(e.target.value)} placeholder="Your Name *"
            style={{width:"100%",padding:"14px 16px",borderRadius:10,border:`1px solid ${T.border}`,fontSize:14,fontFamily:"inherit",outline:"none",boxSizing:"border-box",color:T.text,marginBottom:10}}/>
          <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="Email *" type="email"
            style={{width:"100%",padding:"14px 16px",borderRadius:10,border:`1px solid ${T.border}`,fontSize:14,fontFamily:"inherit",outline:"none",boxSizing:"border-box",color:T.text,marginBottom:10}}/>
          <input value={phone} onChange={e=>setPhone(e.target.value)} placeholder="Phone (optional)" type="tel"
            style={{width:"100%",padding:"14px 16px",borderRadius:10,border:`1px solid ${T.border}`,fontSize:14,fontFamily:"inherit",outline:"none",boxSizing:"border-box",color:T.text}}/>
          {error && <div style={{fontSize:13,color:T.neg,marginTop:8}}>{error}</div>}
          <button onClick={handleIntelSubmit} disabled={submitting} style={{
            marginTop:12,padding:"14px",borderRadius:12,border:"none",fontSize:15,fontWeight:700,
            background:T.brand,color:"white",cursor:"pointer",width:"100%",opacity:submitting?0.6:1,
          }}>{submitting ? "Generating..." : "View My Report"}</button>
          <button onClick={()=>setShowCapture(false)} style={{marginTop:8,padding:"10px",borderRadius:10,border:"none",background:"transparent",color:T.muted,fontSize:13,cursor:"pointer",width:"100%"}}>← Back</button>
        </Card>
      ) : (
        <>
          {/* ── REPORT HEADER ── */}
          ...
          <Card style={{padding:"20px 28px",background:"#f0fdf4",border:`1px solid #bbf7d0`}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}>
              <div>
                ...
                <div style={{fontSize:12,color:T.sec}}>David Rose · HomeSmart Realty West · DRE# 02168977 · TheEncinitasReport.com · Data as of {RATES.updated}</div>
              </div>
